...
let onNewDataCallback: ((snapshot: MonitoringSnapshot) => void) | null = null;
  ...
  try {
    ...
    if (hasChanged(latestSnapshot, nextSnapshot)) {
      ...
      if (onNewDataCallback) {
        onNewDataCallback(nextSnapshot);
      }
    } else {
      latestSnapshot = nextSnapshot;
    }
...
export function startOneNETPolling(intervalMs = 3000, onNewData?: (snapshot: MonitoringSnapshot) => void): void {
  if (pollingInterval) {
