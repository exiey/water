...
import { getLatestSnapshot, getSnapshotHistory } from '../onenet-polling.js';
...
function getMonitoringContext(): string {
  ...
  const history = getSnapshotHistory(10);

  if (!latest) {
    return '当前无法获取监测数据，请检查设备连接状态。';
  }

  let context = `## 当前监测数据 (${latest.recorded_at})\n`;
  ...
  // 添加历史趋势
  if (history.length > 1) {
    context += `\n## 最近${history.length}次数据趋势\n`;
    const recent = history.slice(0, 5);
    context += recent.map((h, i) => {
      return `${i + 1}. ${h.recorded_at}: 水位${h.water_level}m, 流量${h.water_flow}m³/s`;
    }).join('\n');
  }
